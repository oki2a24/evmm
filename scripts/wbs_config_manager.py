...
class WBSConfigManager:
    ...
    def infer_structure(self) -> Dict[str, Any]:
        ...
        【推論の規律と再現性の担保】
        1. ヘッダー行の特定: 「機能ID」等のキーワードをスキャンし、基準となる行を特定します。
        2. 全列の抽出 (columns["all"]): 
           テンプレート復元のために、役割（Role）が割り当てられていない列（例：備考）や空列も含め、
           全ての列を順序通りに記録します。これにより「見た目の完全な再現」を可能にしています。
        3. フェーズの分離: ヘッダー行の 1行上のデータをフェーズ見出しとして扱います。
           ヘッダーが空であっても、フェーズ行に名前があれば新しいフェーズの開始とみなします。
        """
