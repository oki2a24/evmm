...
- [ ] **ステップ 5: code-reviewer による設計・規律のレビュー**
- [ ] **ステップ 6: コミット**
...
- [ ] **ステップ 4: テストと手動検証**
- [ ] **ステップ 5: code-reviewer による対話フローのUI/UXレビュー**
- [ ] **ステップ 6: コミット**
...
- [ ] **ステップ 3: code-reviewer によるリファクタリングの妥当性レビュー**
- [ ] **ステップ 4: コミット**
...
- [ ] **ステップ 4: code-reviewer による全体的な整合性レビュー**
- [ ] **ステップ 5: コミット**
...
トークン効率と品質を両立させるため、以下のフローで実行します。

1. **インライン実行**: `executing-plans` を使用し、メインセッションで直接実装を行います（トークン節約）。
2. **ゲート付きレビュー**: 各タスクの「code-reviewer によるレビュー」ステップにおいて、専門のレビューエージェントを呼び出し、客観的な品質チェックを行います。
3. **継続的な検証**: 各ステップごとにテストを実行し、物理的な証拠（GREEN）を確認しながら進めます。
